...
- `/session`と`/callback`は各10回/分、`/exchange`は20回/分の独立したCloudflare Rate Limiting
  bindingで接続元ごとに制限する。これは認証の代替ではなく、state・handoff code・端末秘密値の検証も必須
- `/callback`の開始時にDurable Object内の状態をtransactionで`registered`から`exchanging`へ一度だけ
  遷移させる。同じstateの並行callbackはHTTP 409で拒否し、Notion token endpointへは最初の1要求だけ送る
- 認可code交換の失敗・キャンセルはセッションを削除し、同じcodeを再試行せず新しいloginを要求する
...
4. Durable Object migrationが`wrangler.toml`に含まれていることを確認する:
   ...
   ```
5. `SESSION_RATE_LIMITER`、`CALLBACK_RATE_LIMITER`、`EXCHANGE_RATE_LIMITER`の`namespace_id`を、
   Cloudflare account内で他のRate Limiting bindingと共有しない正整数文字列へ変更する。意図せず共有すると
   endpoint間や別Workerとカウンタが混在する
